/ 2018.02.03T02:19:17.881 j1 weaves
/ q salting.load.q FILE [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-exit] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] 
/ q salting.load.q FILE
/ q salting.load.q
/ q salting.load.q -help
FILE:LOADFILE:`$":in/salting.csv"
o:.Q.opt .z.x;if[count .Q.x;FILE:hsym`${x[where"\\"=x]:"/";x}first .Q.x]
if[`help in key o;-1"usage: q salting.load.q [FILE(default:in/salting.csv)] [-help] [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] [-exit]\n";exit 1]
SAVEDB:`:csvdb
SAVEPTN:`
...
LOADNAME:`salting
SAVENAME:`salting
LOADFMTS:"JSSISSS*SSSSESS*FSFSSEESCSCCCCECSSESESSCCCCCCCSSSCSEESF"
LOADHDRS:`objectid`assetid`featureid`sitecode`sitename`locality`town`location`roadclass`hierarchy`customer`areaname`distance`roadtype`salttype`comments`distance1`sitename1`sitecode1`town1`source`priority`dupnored`reduced`add2017`assetid1`practical`notpract`criteria`notmet`featurecnt`dupremove`salt201207`update2017`division`roadname`updateno`reason`aroad`busroute`trnsprtint`school`promfacs`emgncysrvs`lalink`vllgemain`notes`ntwrkrecd`salt2018`saltpartid`saltroute`routesect`saltdrop`resilient`shapelength
...
JUSTSYMFMTS:" SS SSS SSSS SS  S SS  S S      SS S SS       SSS S  S "
JUSTSYMHDRS:`assetid`featureid`sitename`locality`town`roadclass`hierarchy`customer`areaname`roadtype`salttype`sitename1`town1`source`reduced`assetid1`salt201207`update2017`roadname`reason`aroad`notes`ntwrkrecd`salt2018`saltroute`resilient
