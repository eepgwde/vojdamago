#!/usr/bin/env python3

# -*- coding: utf-8 -*-

import os
import datetime

import pandas as pd
import numpy as np

import isodtg

import re
import sys

## Ugly file. Uncontrolled text fields removed. Dropped the original date fields.

def main():
    # print("working: " + sys.argv[1])
    nsheet=int(0)
    if len(sys.argv) >= 4:
        nsheet=int(sys.argv[3])

    tbl = pd.read_excel(sys.argv[1], sheet_name=nsheet)

    tbl.drop(['enquiry_desc', 'enquiry_address'], axis=1, inplace=True) 
    tbl = isodtg.apply(tbl, 'enquiry_time', f=isodtg.isodt0)    
    tbl = isodtg.apply(tbl, 'logged_date', f=isodtg.isodt0)    
    tbl = isodtg.apply(tbl, 'follow_up_date', f=isodtg.isodt0)    

    tbl.drop(['enquiry_time', 'logged_date', 'follow_up_date'], 
             axis=1, inplace=True) 

    isodtg.to_csv(tbl, sys.argv[2])

if __name__ == '__main__':
    sys.argv[0] = re.sub(r'(-script\.pyw|\.exe)?$', '', sys.argv[0])
    sys.exit(main())
