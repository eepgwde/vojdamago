/ 2018.02.02T17:45:49.633 j1 weaves
/ q traffic0.load.q FILE [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-exit] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] 
/ q traffic0.load.q FILE
/ q traffic0.load.q
/ q traffic0.load.q -help
FILE:LOADFILE:`$":in/traffic0.csv"
o:.Q.opt .z.x;if[count .Q.x;FILE:hsym`${x[where"\\"=x]:"/";x}first .Q.x]
if[`help in key o;-1"usage: q traffic0.load.q [FILE(default:in/traffic0.csv)] [-help] [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] [-exit]\n";exit 1]
SAVEDB:`:csvdb
SAVEPTN:`
...
LOADNAME:`traffic0
SAVENAME:`traffic0
LOADFMTS:"JSSI**S*SSSSFSSFFFFFFFFCFSSSF*FSFSSDB"
LOADHDRS:`objectid`assetid`featureid`sitecode`sitename`locality`town`location`roadclass`hierarchy`customer`areaname`distance`roadtype`roadname`length`aadtini`adtpct01`adtpct02`adtpct03`adtpct04`adtpct05`adtpct06`groupt2`hccobjin`hcchier`aadtsourc`compkey`aadtupdat`sourcedat`shapeleng`confidence`aadtcomb`aadtsou1`aadtsou2`modifiedda`checked
...
JUSTSYMFMTS:" SS   S SSSS SS          SSS   S SS  "
JUSTSYMHDRS:`assetid`featureid`town`roadclass`hierarchy`customer`areaname`roadtype`roadname`hcchier`aadtsourc`compkey`confidence`aadtsou1`aadtsou2
