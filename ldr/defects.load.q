/ 2018.02.14T19:46:06.952 j1 weaves
/ q defects.load.q FILE [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-exit] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] 
/ q defects.load.q FILE
/ q defects.load.q
/ q defects.load.q -help
FILE:LOADFILE:`$":in/defects.csv"
o:.Q.opt .z.x;if[count .Q.x;FILE:hsym`${x[where"\\"=x]:"/";x}first .Q.x]
if[`help in key o;-1"usage: q defects.load.q [FILE(default:in/defects.csv)] [-help] [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] [-exit]\n";exit 1]
SAVEDB:`:csvdb
SAVEPTN:`
...
LOADNAME:`defects
SAVENAME:`defects
LOADFMTS:"IFSSIFSZSSFFSSFSHSZ"
LOADHDRS:`usrn`assetnumber`street`assetid`defectnumber`supersedesdefect`defectstatus`defectdate`defecttype`defecttypename`defecteasting`defectnorthing`prioritycode`priorityname`jobnumber`statusflag`statuscode`statusname`defectdate0
...
JUSTSYMFMTS:"  SS  S SS  SS S S "
JUSTSYMHDRS:`street`assetid`defectstatus`defecttype`defecttypename`prioritycode`priorityname`statusflag`statusname
