/ 2018.02.08T20:48:12.568 j1 weaves
/ q permits.load.q FILE [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-exit] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] 
/ q permits.load.q FILE
/ q permits.load.q
/ q permits.load.q -help
FILE:LOADFILE:`$":in/permits.csv"
o:.Q.opt .z.x;if[count .Q.x;FILE:hsym`${x[where"\\"=x]:"/";x}first .Q.x]
if[`help in key o;-1"usage: q permits.load.q [FILE(default:in/permits.csv)] [-help] [-bl|bulkload] [-bs|bulksave] [-js|justsym] [-savedb SAVEDB] [-saveptn SAVEPTN] [-savename SAVENAME] [-exit]\n";exit 1]
SAVEDB:`:csvdb
SAVEPTN:`
...
LOADNAME:`permits
SAVENAME:`permits
LOADFMTS:"ISSSSSSSSSIESSSHSSBCDVCDCDSCECC"
LOADHDRS:`workheader`worksreference`worksstate`district`areaname`promotername`organisationname`organisationcode`promotertype`promoterutility`usrn`streetcategory`trafficmanagement`trafficsensitive`notificationtype`seqno`workscategory`actionofficer`permitfee`coordinatortimevalue`noticeissuedate`noticeissuetime`startdateproposed`startdateactual`enddateestimated`enddateactual`notificationintime`notificationleadtimeinitialapplication`duration`workingdays`responsecode
...
JUSTSYMFMTS:" SSSSSSSSS  SSS SS        S    "
JUSTSYMHDRS:`worksreference`worksstate`district`areaname`promotername`organisationname`organisationcode`promotertype`promoterutility`trafficmanagement`trafficsensitive`notificationtype`workscategory`actionofficer`notificationintime
