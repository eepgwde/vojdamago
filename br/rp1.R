...
## The most important feature is that most claims are local roads.
...
fit0[["control"]] <- rpart.control(maxcompete=fit0[["maxcompete"]], cp=fit0[["cp"]], 
                                   xval=10, maxdepth=10, surrogatestyle = 1)

# fit <- rpart(fmla, method = "class", control=fit0[["control"]], data = ppl1 )
fit <- rpart(fmla, method = fit0[["method"]], control=fit0[["control"]], data = ppl1 )
